...
shinyServer(function(input, output, session) {
  if (!interactive()) {
    if (file.exists("/tmp/FVSOnline.log")) {
      unlink("/tmp/FVSOnline.older.log")
      file.rename("/tmp/FVSOnline.log", "/tmp/FVSOnline.older.log")
    }
    # make sure the sink stack is empty
    while (sink.number()) sink()
    sink("/tmp/FVSOnline.log")
  }
  ...
  withProgress(session, {
    ...
    if (file.exists("FVS_Runs.RData")) {
      ...
      if (!is.null(attr(FVS_Runs, "stdstkParms"))) {
        ...
        if (!is.na(val)) {
          updateNumericInput(
            session = session, inputId = "sdskldbh",
            value = val
          )
        }
      }
      cat(file = stderr(),"length(FVS_Runs)=", length(FVS_Runs), " length(notok)=", length(notok), " notok=", notok, "\n")
      ...
      cat(file = stderr(),"serious start up error\n")
      return()
    }
    setProgress(
      message = "Start up",
    ...
    if (!file.exists("projectId.txt")) {
      cat(file = stderr(),"title= ", basename(getwd()), "\n", file = "projectId.txt")
    }
    ...
    cat(file = stderr(),"Setting initial selections, length(selChoices)=", length(selChoices), "\n")
    updateSelectInput(
      session = session, inputId = "runSel",
  ...
  session$onSessionEnded(function() {
    cat(file = stderr(),
      "onSessionEnded, globals$saveOnExit=", globals$saveOnExit,
    ...
    if (globals$saveOnExit) {
      saveRun()
      FVS_Runs <- globals$FVS_Runs
      attr(FVS_Runs, "stdstkParms") <- isolate(list(
        "sdskwdbh" = input$sdskwdbh,
  ...
  initTableGraphTools <- function() {
    cat(file = stderr(),"initTableGraphTools\n")
  ...
  observe({
    if (input$topPan == "View Outputs" && input$leftPan == "Load") {
      cat(file = stderr(),"View Outputs & Load\n")
      initTableGraphTools()
      ...
      if (length(tbs) > 0 && !is.na(match("FVS_Cases", tbs))) {
        runsdf <- dbGetQuery(
          dbGlb$dbOcon,
          paste0(
            "Select RunTitle,KeywordFile from FVS_Cases group by KeywordFile ",
      ...
      updateSelectInput(session, "runs",
        choices = fvsOutData$runs,
        selected = 0
      )
    }
  })
  ...
  observe({
    if (input$leftPan != "Load") {
      return()
    }
    cat(file = stderr(),"runs, run selection (load) input$runs=", input$runs, "\n")
    if (!is.null(input$runs)) # will be a list of run keywordfile names (uuid's)
    {
      tbs <- dbGetQuery(dbGlb$dbOcon, "select name from sqlite_master where type='table';")[, 1]
      cat(file = stderr(),"tbs related to the run", tbs, "\n")
      if (length(tbs) == 0) {
      ...
      withProgress(session, {
        i <- 1
        setProgress(
          message = "Output query",
          detail = "Selecting tables", value = i
        )
        i <- i + 1
        ...
        while (TRUE) {
          trycnt <- trycnt + 1
          setProgress(
            message = "Getting exclusive lock",
          ...
          cat(file = stderr(),"try to get exclusive lock, trycnt=", trycnt, "\n")
        ...
        dbExecute(dbGlb$dbOcon, paste0(
          "create table temp.Cases as select CaseID ",
          "from FVS_Cases where FVS_Cases.KeywordFile in ", inSet
        ))
        for (tb in tbs)
        {
          cat(file = stderr(),"tb=", tb, "\n")
          cnt <- 0
          if (tb == "FVS_Cases") next
          if (tb %in% c(
            "CmpSummary", "CmpSummary_East",
            ...
            cnt <- if ("CaseID" %in% dbListFields(dbGlb$dbOcon, tb)) {
              dbGetQuery(dbGlb$dbOcon, paste0(
                "select count(*) from ",
        ...
        if ("FVS_Compute" %in% tbs && ncases > 1) {
          ...
          sumExpressions <- paste0(
            lapply(
              setdiff(colnames(cmp), c("CaseID", "StandID", "Year")),
              function(var) paste0(
                  "round(sum(", var,
                  "*SamplingWT)/sum(SamplingWt),2) as Cmp", var
                )
            ),
            collapse = ","
          )
        ...
        setProgress(
          message = "Output query",
          detail = "Committing changes", value = i
        )
        i <- i + 1
        ...
        if (!is.null(dbd[["FVS_Summary_East"]])) {
          dbd$FVS_Summary_East <-
        ...
        if (!is.null(dbd[["CmpSummary_East"]])) {
          dbd$CmpSummary <- c(
            dbd$CmpSummary_East,
            c("CmpTPrdTpa", "CmpTPrdTCuFt", "CmpTPrdMCuFt", "CmpTPrdBdFt")
          )
  ...
  observe({
    ...
    if (is.null(input$selectdbtables)) {
      ...
      updateSelectInput(
        session = session, "selectdbvars", choices = as.list(vars),
        selected = vars
      )
    }
  })
  ...
  observe({
    if (!is.null(input$selectdbvars)) {
      cat(file = stderr(),"selectdbvars input$selectdbvars", input$selectdbvars, "\n")
  ...
  observe({
    if (input$leftPan == "Custom Query") {
      ...
      if (length(globals$customQueries) == 0) {
        updateSelectInput(
          session = session, inputId = "sqlSel",
        ...
        updateSelectInput(
          session = session, inputId = "sqlSel", choices = sels,
          selected = 0
        )
      }
    ...
    if (input$sqlRunQuery > 0) {
      ...
      isolate({
        ...
        for (qry in qrys)
        {
          if (nchar(qry)) {
            iq <- iq + 1
            cat(file = stderr(),"sqlRunQuery, qry=", qry, "\n")
            res <- try(dbGetQuery(dbGlb$dbOcon, qry))
            msgtxt <- if (class(res) == "data.frame" && ncol(res) && nrow(res)) {
              paste0(
                msgtxt, "query ", iq, " returned a data frame with ", nrow(res),
            ...
            updateTextInput(
              session = session, inputId = "sqlOutput", label = "",
              value = msgtxt
            )
  ...
  observe({
    cat(file = stderr(),
      "sqlSel input$sqlSel=", input$sqlSel, " isnull=",
    ...
    if (!is.null(input$sqlSel)) {
      ...
      cat(file = stderr(),"sqlSel sel=", sel, "\n")
      if (length(globals$customQueries) >= sel) {
        updateTextInput(
          session = session, inputId = "sqlTitle",
          value = names(globals$customQueries)[sel]
        )
        updateTextInput(
          session = session, inputId = "sqlQuery",
  ...
  observe({
    if (input$sqlDelete > 0) {
      isolate({
        cat(file = stderr(),"sqlDelete is.null(input$sqlTitle)=", is.null(input$sqlTitle), "\n")
        if (is.null(input$sqlTitle)) {
          return()
        }
  ...
  observe({
    if (input$leftPan == "Explore") {
      cat(file = stderr(),"Explore, length(fvsOutData$dbSelVars)=", length(fvsOutData$dbSelVars), "\n")
      ...
      withProgress(session, {
        iprg <- 1
        setProgress(
          message = "Processing variable names", detail = "",
          value = iprg
        )
        tbs <- unique(unlist(lapply(
          strsplit(fvsOutData$dbSelVars, ".", fixed = TRUE),
          function(x) x[1]
        )))
        if (length(tbs) == 0) {
          return()
        ...
        tbgroup <- c(
          "CmpMetaData" = "0", "CmpSummary" = 1, "CmpSummary_East" = 1,
        ...
        {
          cat(file = stderr(),"tb=", tb, " len(dat)=", length(dat), "\n")
          iprg <- iprg + 1
          ...
          if (tb %in% c("CmpSummary", "CmpSummary_East", "CmpSummary2", "CmpSummary2_East")) {
            ...
            for (col in colnames(dtab)) {
              if (is.character(dtab[, col])) {
        ...
        cat(file = stderr(),"Explore, len(dat)=", length(dat), "\n")
        if (length(dat) == 0) {
          initTableGraphTools()
          return()
        }
        ...
        for (tb in names(dat))
        {
          ...
          if (tb %in% c(
            "FVS_TreeList", "FVS_ATRTList", "FVS_CutList",
          ...
          cat(file = stderr(),"tb=", tb, " is.null(mdat)=", is.null(mdat), "\n")
          if (is.null(mdat)) {
            mdat <- dat[[tb]]
          } else {
            ...
            setProgress(
              message = "Merging selected tables",
              detail = tb, value = iprg
            )
            cat(file = stderr(),"tb=", tb, " mrgVars=", mrgVars, "\n")
        ...
        vars <- colnames(mdat)
        sby <- intersect(vars, c(
          "MgmtID", "StandID", "Stand_CN", "Year", "PtIndex",
          "TreeIndex", "Species", "DBHClass"
        ))
        sby <- if (length(sby)) {
          cmd <- paste0(
            "order(", paste(paste0("mdat$", sby), collapse = ","),
            if ("srtOrd" %in% vars) ",mdat$srtOrd)" else ")"
          )
          cat(file = stderr(),"cmd=", cmd, "\n")
        ...
        vars <- intersect(c(
          "MgmtID", "Stand_CN", "StandID", "Year",
          "Species", "DBHClass"
        ), colnames(mdat))
        vars <- c(vars, setdiff(colnames(mdat), vars))
        endvars <- intersect(c(
          "SamplingWt", "Variant", "RunTitle",
          "Groups", "RunDateTime", "KeywordFile", "CaseID"
        ), vars)
        ...
        if (is.null(mdat$StandID)) {
          ...
          if (length(levels(mdat$StandID)) > 500) {
            updateSelectInput(session, "stdid",
              choices = as.list(paste0(
                "None loaded (",
                length(levels(mdat$StandID)), " stands)"
              )), selected = NULL
            )
          } else {
            updateSelectInput(session, "stdid",
        ...
        if (is.null(mdat$Groups)) {
          updateSelectInput(session, "stdgroups",
            choices = list("None loaded"), selected = NULL
          )
        } else {
          grps <- sort(unique(unlist(lapply(levels(mdat$Groups), function(x)
            trim(scan(text = x, what = "character", sep = ",", quiet = TRUE))))))
          updateSelectInput(session, "stdgroups",
            choices = as.list(grps),
            selected = grps
          )
        }
        if (is.null(mdat$MgmtID)) {
          updateSelectInput(session, "mgmid",
            choices = list("None loaded"), selected = NULL
          )
        } else {
          updateSelectInput(session, "mgmid",
            choices = as.list(levels(mdat$MgmtID)),
            selected = levels(mdat$MgmtID)
          )
        }
        if (length(intersect(c(
          "FVS_TreeList", "FVS_ATRTList", "FVS_CutList",
          "FVS_TreeList_East", "FVS_ATRTList_East", "FVS_CutList_East"
        ), names(dat)))) {
          updateSelectInput(session, "plotType", selected = "scat")
        } else
        if (length(intersect(c("StdStk", "CmpStdStk", "StdStk_East", "CmpStdStk_East"), names(dat)))) {
          updateSelectInput(session, "plotType", selected = "bar")
        } else {
        ...
        if (is.null(mdat$Year)) {
          ...
          sel <- if (length(intersect(c(
            "FVS_TreeList", "FVS_ATRTList", "FVS_CutList",
            "FVS_TreeList_East", "FVS_ATRTList_East", "FVS_CutList_East",
            "StdStk", "StdStk_East", "CmpStdStk", "CmpStdStk_East"
          ), names(dat)))) {
            sel[isel]
          } else {
            sel
          ...
          updateSelectInput(session, "year",
            choices = as.list(levels(mdat$Year)),
            selected = sel
          )
        }
        ...
        selVars <- unlist(lapply(
          c(
            "StandID", "MgmtID", "Year", "^DBH", "^DG$",
        ...
        updateCheckboxGroupInput(session, "browsevars",
  ...
  renderTable <- function(dat) {
    cat(file = stderr(),"renderTable, is.null=", is.null(dat), " nrow(dat)=", nrow(dat), "\n")
    ...
    renderRHandsontable(if (is.null(dat) || nrow(dat) == 0) {
      NULL
    } else {
      rhandsontable(dat,
  ...
  observe({
    ...
    cat(file = stderr(),"filterRows and/or pivot\n")
    ...
    dat <- if (length(input$stdtitle) || length(input$stdgroups) ||
      ...
      fvsOutData$dbData[filterRows(
        fvsOutData$dbData, input$stdtitle, input$stdgroups,
        input$stdid, input$mgmid, input$year, input$species, input$dbhclass
      ),
      fvsOutData$browseSelVars,
      drop = FALSE
      ]
    } else {
  ...
  observe({
    if (!is.null(input$browsevars) && !is.null(input$plotType)) {
      cat(file = stderr(),"browsevars/plotType\n")
      fvsOutData$browseSelVars <- input$browsevars
      ...
      updateSelectInput(session, "pivVar",
        choices = as.list(c("None", cats)),
        selected = spiv
      )
      updateSelectInput(session, "dispVar",
        choices = as.list(ccont),
        selected = sdisp
      )
      isolate({
        curX <- input$xaxis
        ...
        updateSelectInput(
          session = session, inputId = "hfacet", choices = as.list(c("None", cats)),
          selected = sel
        )
        sel <- if (length(intersect(cats, "MgmtID")) > 0) "MgmtID" else "None"
        updateSelectInput(
          session = session, inputId = "vfacet", choices = as.list(c("None", cats)),
          selected = sel
        )
        sel <- if (length(intersect(cats, "Species")) > 0) "Species" else "None"
        updateSelectInput(
          session = session, inputId = "pltby", choices = as.list(c("None", cats)),
          selected = sel
        )
      })
    }
  ...
  observe({
    ...
    isolate({
      if (all(!c(
        is.null(input$pltby), is.null(input$xaxis), is.null(input$pltby),
        is.null(input$yaxis)
      )) &&
  ...
  observe({
    if (is.null(input$vfacet) || input$vfacet == "None") {
      return()
    }
    isolate({
  ...
  observe({
    if (is.null(input$hfacet) || input$hfacet == "None") {
      return()
    }
    isolate({
  ...
  output$outplot <- renderImage({
    ...
    dat <- if (input$leftPan == "Custom Query") {
      fvsOutData$dbData
    } else {
      droplevels(fvsOutData$dbData[filterRows(
        fvsOutData$dbData, input$stdtitle,
    ...
    if (length(setdiff(needVars, names(dat)))) {
      return(nullPlot())
    }
    cat(file = stderr(),"vf=", vf, " hf=", hf, " pb=", pb, " xaxis=", input$xaxis, " yaxis=", input$yaxis, "\n")
    if (is.null(input$xaxis) || is.null(input$yaxis)) {
      return(nullPlot("Select both X- and Y-axes"))
    }
    if (!is.null(hf) && nlevels(dat[, hf]) > 8) {
      cat(file = stderr(),"hf test, nlevels(dat[,hf])=", nlevels(dat[, hf]), "\n")
      return(nullPlot(paste0("Number of horizontal facets= ", nlevels(dat[, hf]), " > 8")))
    }
    if (!is.null(vf) && nlevels(dat[, vf]) > 8) {
      cat(file = stderr(),"vf test hit, nlevels(dat[,vf])=", nlevels(dat[, vf]), "\n")
    ...
    for (v in c("MgmtID", "StandID", "Year"))
    {
      if (v %in% names(dat) && nlevels(dat[[v]]) > 1 &&
        ...
        return(nullPlot(paste0(
          "Variable ", v, " has ", nlevels(dat[[v]]), " levels and ",
    ...
    cat(file = stderr(),"sumOnSpecies=", sumOnSpecies, " sumOnDBHClass=", sumOnDBHClass, "\n")
    if (sumOnSpecies) {
    ...
    if (input$facetWrap == "Off") {
      ...
      fg <- if (is.null(fg) && !is.null(nd$hfacet)) {
        facet_wrap(~hfacet, ncol = ceiling(sqrt(nlevels(nd$hfacet))), strip.position = "top")
      } else {
        fg
      }
      fg <- if (is.null(fg) && !is.null(nd$vfacet)) {
        facet_wrap(~vfacet, ncol = ceiling(sqrt(nlevels(nd$vfacet))), strip.position = "right")
      } else {
        fg
      }
    }
    ...
    colors <- if (input$colBW == "B&W") {
      unlist(lapply(seq(0, .3, .05), function(x) rgb(x, x, x)))
    } else {
      if (is.null(input$color1)) {
        cbbPalette
      } else {
        c(
          input$color1, input$color2, input$color3, input$color4, input$color5,
          input$color6
        )
      }
    }
    ...
    cat(file = stderr(),"nlevels=", nlevels(nd$Legend), " colors=", colors, "\n")
    p <- p + theme(axis.text.x = element_text(
      angle = as.numeric(input$XlabRot),
      hjust = if (input$XlabRot == "45") 1 else .5
    ))
    p <- p + theme(axis.text.y = element_text(
      angle = as.numeric(input$YlabRot),
    ...
    cat(file = stderr(),"input$XTrans=", input$XTrans, " input$YTrans=", input$YTrans, "\n")
    ...
    cat(file = stderr(),"ylim=", ylim, " xlim=", xlim, "\n")
    ymaxlim <- NA
    xmaxlim <- NA
    DMDguideLines <- NULL
    if (isolate(input$plotType) == "DMD") {
      ...
      if (!is.null(sdis)) {
        ...
        if (maxSDI == -Inf) {
          maxSDI <- 700
          sdis <- c(sdis, as.character(maxSDI))
        }
        sdisn <- NULL
        for (xx in sdis)
        {
          ...
          nv <- if (li > 1 && substr(xx, li, li) == "%") {
            as.numeric(substr(xx, 1, li - 1)) * .01 * maxSDI
          } else {
            as.numeric(xx)
          }
          sdisn <- c(sdisn, nv)
        }
        cat(file = stderr(),"sdisn=", sdisn, "\nXUnits=", input$XUnits, " YUnits=", input$YUnits, "\n")
        ...
        for (SDI in sdisn)
        {
          ...
          cat(file = stderr(),"SDI=", SDI, " ymaxlim=", ymaxlim, " xmaxlim=", xmaxlim, "\n")
        }
      }
    }
    ...
    if (isolate(input$plotType) == "StkCht") {
      ...
      if (length(sdis)) {
        ...
        for (i in 1:length(sdis)) {
          yptsba <- c(
            70.2, 80.9, 89.5, 96.5, 102.5, 107.5, 111.9, 115.7, 119.0, 121.8,
            124.4, 126.6, 128.9
          )
        }
        ...
        pm <- min(pcts)
        px <- max(pcts)
        StkChtrng <- data.frame(
          X = c(xptstpa[1] * pm, xptstpa[1] * px, xptstpa * px, rev(xptstpa) * pm),
    ...
    cat(file = stderr(),"pltp=", pltp, " input$colBW=", input$colBW, " hrvFlag is null=", is.null(hrvFlag), "\n")
    ...
    if (!is.factor(nd$X)) {
      ...
      cat(file = stderr(),"xlim=", xlim, " rngx=", rngx, " brkx=", brkx, "\n")
    }
    if (!is.factor(nd$Y)) {
      ...
      cat(file = stderr(),"ylim=", ylim, " rngy=", rngy, " brky=", brky, "\n")
    }
    ...
    if (!is.null(DMDguideLines)) {
      ...
      for (linetype in 1:length(pltorder))
      {
        ...
        p <- p + geom_line(aes(x = xseq, y = yseq),
      ...
      for (idrow in seq(1, nrow(guidedf) - 1, 2))
      {
        linetype <- linetype + 1
        p <- p + annotate(
          geom = "text", hjust = "left",
          label = paste0(guidedf$SDI[idrow]), size = 2, y = guidedf$ys[idrow], x = guidedf$xs[idrow + 1]
        ) +
          annotate("segment",
            y = guidedf$ys[idrow], yend = guidedf$ys[idrow + 1], linetype = linetype,
    ...
    if (!is.null(StkChtguideLines)) {
      ...
      for (idrow in seq(1, nrow(guidedf) - 1, 2))
      {
        ...
        p <- p + annotate(
          ...
          annotate("segment",
            y = guidedf$ys[idrow], yend = guidedf$ys[idrow + 1], alpha = .4,
            x = guidedf$xs[idrow], xend = guidedf$xs[idrow + 1],
            linetype = if (guidedf$PCT[idrow] == "100%") 1 else linetype
          )
      }
      p <- p + geom_polygon(aes(x = X, y = Y),
        data = StkChtrng, color = "Gray", alpha = .3,
        show.legend = FALSE
      )
    }
    ...
    p <- p + switch(pltp,
      line = if (input$colBW == "B&W") {
        geom_line(aes(x = X, y = Y, linetype = Legend), alpha = alpha)
      } else {
        geom_line(aes(x = X, y = Y, color = Legend), alpha = alpha)
      },
      path = if (input$colBW == "B&W") {
        geom_path(aes(x = X, y = Y, linetype = Legend),
          alpha = alpha,
          arrow = grid::arrow(
            angle = 20, length = unit(6, "pt"),
            ends = "last", type = "closed"
          )
        )
      } else {
        geom_path(aes(x = X, y = Y, color = Legend),
          alpha = alpha,
          arrow = grid::arrow(
            angle = 20, length = unit(6, "pt"),
            ends = "last", type = "closed"
      ...
      bar = if (input$colBW == "B&W") {
        geom_col(aes(x = X, y = Y, fill = Legend),
          color = "black", size = .2, alpha = alpha,
          position = input$barPlace
        )
      } else {
        geom_col(aes(x = X, y = Y, fill = Legend),
          color = "transparent", size = .1, alpha = alpha,
          position = input$barPlace
        )
      },
      box = if (input$colBW == "B&W") {
        geom_boxplot(aes(x = X, y = Y, linetype = Legend), color = "black", size = .6, alpha = alpha)
      } else {
        geom_boxplot(aes(x = X, y = Y, color = Legend), linetype = 1, size = .6, alpha = alpha)
      }
    )
    if (!is.null(hrvFlag) && any(hrvFlag)) {
      p <- p +
        if (input$colBW == "B&W") {
          geom_point(aes(x = X, y = Y),
            shape = 82, # the letter R is code 82
            data = nd[hrvFlag, ], alpha = alpha, show.legend = FALSE
          )
        } else {
          geom_point(aes(x = X, y = Y, color = Legend),
            shape = 82, # the letter R is code 82
            data = nd[hrvFlag, ], alpha = alpha, show.legend = FALSE
          )
        }
    ...
    p <- p + switch(pltp,
      ...
      scat =
        guides(shape = guide_legend(
          override.aes = list(color = colors, alpha = 1, size = 1),
          title = legendTitle
        ), color = "none"),
      bar =
        guides(fill = guide_legend(
          override.aes = list(alpha = .9, size = .6),
          title = legendTitle, keywidth = .8, keyheight = .8
        )),
      box = if (input$colBW == "B&W") {
        guides(linetype = guide_legend(
          override.aes = list(alpha = .8, size = .5),
          title = legendTitle, keywidth = .8, keyheight = .8
        ))
      } else {
    ...
    if (nlevels(nd$Legend) == 1 || nlevels(nd$Legend) > 30) {
      ...
      p <- p + theme(legend.position = input$legendPlace)
    }
    outfile <- "plot.png"
    ...
    png(outfile,
      width = fvsOutData$plotSpecs$width,
  ...
  updateStandTableSelection <- function() {
    cat(file = stderr(),"in updateStandTableSelection\n")
    updateSelectInput(
      session = session, inputId = "inTabs", choices = globals$selStandTableList,
  ...
  observe({
    ...
    cat(file = stderr(),"inTabs\n")
  })

  updateVarSelection <- function() {
    cat(file = stderr(),"in updateVarSelection\n")
    if (length(globals$fvsRun$FVSpgm) == 0) {
      selVarListUse <- intersect(
        names(globals$selVarList),
    ...
    updateSelectInput(
      session = session, inputId = "inVars", choices = vlst,
  ...
  observe({
    ...
    reloadStandSelection()
    cat(file = stderr(),"inVars\n")
  })

  reloadStandSelection <- function() {
    cat(file = stderr(),"in reloadStandSelection\n")
    stdInit <- NULL
    ...
    plotInit <- NULL
    for (i in 1:length(dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[, 1])) {
      if (!is.na(match(toupper(dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[[1]][i]), toupper("FVS_PlotInit")))) {
        plotInit <- dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[[1]][i]
      }
    }
    stdInit_cond <- NULL
    for (i in 1:length(dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[, 1])) {
    ...
    stdInit_plot <- NULL
    for (i in 1:length(dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[, 1])) {
      if (!is.na(match(toupper(dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[[1]][i]), toupper("FVS_StandInit_Plot")))) {
        stdInit_plot <- dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[[1]][i]
      }
    }
    plotInit_plot <- NULL
    for (i in 1:length(dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[, 1])) {
      if (!is.na(match(toupper(dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[[1]][i]), toupper("FVS_PlotInit_Plot")))) {
        plotInit_plot <- dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[[1]][i]
      }
    }
    grps <- if (!is.na(match(input$inTabs, "Stands (FVS_StandInit)")) && !is.null(input$inTabs)) {
      try(dbGetQuery(dbGlb$dbIcon, paste0(
        "select Stand_ID,Groups from ",
      ...
      try(dbGetQuery(dbGlb$dbIcon, paste0(
        "select StandPlot_ID,Groups from ",
    ...
    } else if (!is.na(match(input$inTabs, "Inventory Subplots (FVS_PlotInit_Plot)(e.g.: FIA subplots)")) && !is.null(input$inTabs)) {
      try(dbGetQuery(dbGlb$dbIcon, paste0(
        "select StandPlot_ID,Groups from ",
    ...
    if (class(grps) == "try-error" || is.null(grps) || nrow(grps) == 0) {
      ...
      if (!is.na(match(input$inTabs, "Stands (FVS_StandInit)")) ||
        ...
        colnames(dd) <- c("StandPlot_ID", "Grp")
      }
      dd <- as.data.frame(dd)
      dbExecute(dbGlb$dbIcon, "drop table if exists temp.Grps")
      ...
      selGrp <- dbGetQuery(
        dbGlb$dbIcon,
        "select distinct Grp from temp.Grps order by Grp"
      )[, 1]
      updateSelectInput(
        session = session, inputId = "inGrps",
        choices = as.list(selGrp)
      )
      updateSelectInput(
        session = session, inputId = "inStds",
        choices = list()
      )
  ...
  observe({
    if (input$topPan == "Runs" || input$rightPan == "Stands") {
      cat(file = stderr(),"inGrps inAnyAll inStdFindBut\n")
      stdInit <- NULL
      for (i in 1:length(dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[, 1])) {
        if (!is.na(match(toupper(dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[[1]][i]), toupper("FVS_StandInit")))) {
          stdInit <- dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[[1]][i]
      ...
      if (is.null(input$inGrps)) {
        ...
        dbWriteTable(
          dbGlb$dbIcon, DBI::SQL("temp.SGrps"),
        ...
        if ((!is.na(match(input$inTabs, "Stands (FVS_StandInit)")) && !is.null(input$inTabs)) ||
          ...
          stds <- try(dbGetQuery(dbGlb$dbIcon, paste0(
            "select Stand_ID from temp.Grps ",
            "where Grp in (select SelGrps from temp.SGrps)"
          )))
        } else {
          stds <- try(dbGetQuery(dbGlb$dbIcon, paste0(
            "select StandPlot_ID from temp.Grps ",
            "where Grp in (select SelGrps from temp.SGrps)"
          )))
        }
        if (class(stds) == "try-error") {
          return()
        }
        cat(file = stderr(),"inGrps, nrow(stds)=", nrow(stds), "\n")
        globals$selStds <- stds[, 1]
        globals$selStds <- if (input$inAnyAll == "Any") {
          unique(globals$selStds)
        } else {
          stdCnts <- table(globals$selStds)
          names(stdCnts[stdCnts == length(input$inGrps)])
        }
        isolate({
          cat(file = stderr(),"input$inStdFind=", input$inStdFind, "\n")
          srchStr <- input$inStdFind
          if (length(globals$selStds) && nchar(srchStr)) {
            globals$selStds <-
        ...
        stds <- if (length(globals$selStds) <= 220) {
          globals$selStds
        } else {
          c(globals$selStds[1:200], paste0(
            "<< Display 201 to ",
  ...
  ## inStds has changed
  observe({
    cat(file = stderr(),"inStds, length(input$inStds)=", length(input$inStds), "\n")
    if (length(input$inStds) != 1) {
      return()
    }
    ...
    cat(file = stderr(),"inStds, nprts=", nprts, "\n")
    up <- nprts[c(1, 2)] - 200
    ...
    dnM <- if (dn[1] <= length(globals$selStds)) {
      paste0(
        "<< Display ", dn[1], " to ",
        dn[2], " of ", length(globals$selStds), " >>"
      )
    } else {
      NULL
    }
    stds <- c(upM, globals$selStds[nprts[1]:nprts[2]], dnM)
    cat(file = stderr(),"inStds upM=", upM, " dnM=", dnM, "\n")
    updateSelectInput(
      session = session, inputId = "inStds",
      choices = as.list(stds)
    )
  ...
  observe({
    if (input$newRun > 0) {
      ...
      loadVarData(globals, prms, dbGlb$dbIcon)
      updateTextInput(
        session = session, inputId = "title", label = "Run title",
        value = globals$fvsRun$title
      )
      updateTextInput(
        session = session, inputId = "defMgmtID",
        value = globals$fvsRun$defMgmtID
      )
      updateSelectInput(
        session = session, inputId = "simCont",
        choices = list(), selected = NULL
      )
      output$contCnts <- renderUI(HTML(paste0(
        "<b>Contents</b><br>",
        length(globals$fvsRun$stands), " stand(s)<br>",
        length(globals$fvsRun$grps), " group(s)"
      )))
      updateSelectInput(
        session = session, inputId = "addMgmtCats",
        choices = list(), selected = NULL
      )
      updateSelectInput(
        session = session, inputId = "addMgmtCmps",
        choices = list(), selected = NULL
      )
      updateTextInput(
        session = session, inputId = "startyr",
        value = globals$fvsRun$startyr
      )
      updateTextInput(
        session = session, inputId = "endyr",
        value = globals$fvsRun$endyr
      )
      updateTextInput(
        session = session, inputId = "cyclelen",
        value = globals$fvsRun$cyclelen
      )
      updateTextInput(
        session = session, inputId = "cycleat",
      ...
      updateSelectInput(
        session = session, inputId = "compTabSet",
        selected = "Management"
      )
      updateSelectInput(
        session = session, inputId = "runScript",
        selected = "fvsRun"
      )
      isolate({
        if (!is.null(input$inVars) && !is.null(input$inTabs)) {
          ...
          if (length(selVarListUse)) {
            ...
            names(vlst) <- selVarListUse
            updateSelectInput(
              session = session, inputId = "inVars", NULL,
              vlst, vlst[[1]]
            )
          } else {
            updateSelectInput(session = session, inputId = "inVars", NULL, list())
          }
          ...
          if (input$rightPan != "Stands") {
            updateTabsetPanel(
              session = session, inputId = "rightPan",
              selected = "Stands"
            )
          }
        }
  ...
  observe({
    if (input$reload > 0 || !is.null(input$runSel)) {
      cat(file = stderr(),
        "reload or run selection, runSel=", input$runSel, " lensim=",
      ...
      if (length(globals$currentQuickPlot) &&
        ...
        cat(file = stderr(),"setting uiRunPlot to NULL\n")
      ...
      if (class(ret) == "try-error") {
        cat(file = stderr(),"error loading", fn, "\n")
        unlink(fn)
      ...
      if (length(saveFvsRun$stands)) {
        for (i in 1:length(saveFvsRun$stands))
        {
                ...
                for (k in 1:length(saveFvsRun$stands[[i]]$grps[[j]]$cmps))
                {
                  ...
                  if (length(spgtest)) {
                    ...
                    spgname[cntr] <- trim(unlist(strsplit(strsplit(test, split = "\n")[[1]][1],
                      split = " "
                    ))[length(unlist(strsplit(strsplit(test, split = "\n")[[1]][1], split = " ")))])
          ...
          updateSelectInput(
            session = session, inputId = "inTabs", choices = globals$selStandTableList[[1]],
            selected = if (length(globals$selStandTableList)) globals$selStandTableList[[1]] else NULL
          )
        }
      }
      ...
      cat(file = stderr(),"reloaded globals$fvsRun$title=", globals$fvsRun$title, " uuid=", globals$fvsRun$uuid, "\n")
      cat(file = stderr(),"reloaded globals$fvsRun$runScript=", globals$fvsRun$runScript, "\n")
      if (length(globals$fvsRun$uiCustomRunOps)) {
        lapply(names(globals$fvsRun$uiCustomRunOps), function(x, y)
          cat(file = stderr(),"globals$fvsRun$uiCustomRunOps$", x, "=", y[[x]], "\n", sep = ""), globals$fvsRun$uiCustomRunOps)
      } else {
        cat(file = stderr(),"globals$fvsRun$uiCustomRunOps is empty\n")
      }
      ...
      progress$set(
        message = paste0("Setting values for run ", globals$fvsRun$title),
        value = 2
      )
      updateCheckboxGroupInput(
        session = session, inputId = "autoOut",
      ...
      progress$set(message = paste0(
        "Setting simulation contents for run ",
        globals$fvsRun$title
      ), value = 3)
      updateSelectInput(
        session = session, inputId = "simCont",
  ...
  ## autoOut
  observe({
    if (!length(input$simCont) || length(globals$fvsRun$autoOut) == length(input$autoOut)) {
      return()
    }
  ...
  observe({
    if (input$inAdd > 0) {
      cat(file = stderr(),"input$inAdd=", input$inAdd, "\n")
  ...
  observe({
    if (input$inAddGrp > 0) {
      cat(file = stderr()," input$inAddGrp=", input$inAddGrp, "\n")
  ...
  observe({
    if (input$inStdFindBut > 0) {
      cat(file = stderr(),"input$inStdFindBut=", input$inStdFindBut, "\n")
    }
  })
  ...
  observe({
    ...
    cat(file = stderr(),"run element selection\n")
    if (all(input$simCont == globals$fvsRun$selsim)) {
      return()
    }
  ...
  observe({
    if (input$searchNext == 0) {
      return()
    }
    isolate({
      cat(file = stderr(),"searchNext: string=", input$searchString, "\n")
      if (nchar(input$searchString) == 0) {
        return()
      }
      elt <- findStand(globals, search = input$searchString)
      cat(file = stderr(),"elt=", elt, "\n")
      if (is.null(elt)) {
        return()
      }
  ...
  observe({
    ...
    isolate({
      ...
      if (is.null(eltList)) {
        eltList <-
          mkFreeformEltList(
            globals, prms, globals$currentEditCmp$title,
  ...
  observe({
    if (length(input$freeEdit)) {
      # cat(file = stderr(),"call sendcustomMessage\n")
  ...
  insertStrinIntoFreeEdit <- function(string) {
    if (is.null(string) || nchar(string) == 0 || string == " ") {
      return()
    }
    isolate({
      ...
      cat(file = stderr(),"insertStrinIntoFreeEdit string=", string, " start=", start, " end=", end, " len=", len, "\n")
      if (nchar(string) == 0) {
        return()
      }
      if (start == end && end == len) { # prepend
        ...
        str <- input$freeEdit
        updateTextInput(session, "freeEdit",
          value =
            paste0(
              substring(input$freeEdit, 1, max(1, start)), string,
  ...
  observe({
    ...
    isolate({
      cat(file = stderr(),"Cut length(input$simCont) = ", length(input$simCont), "\n")
      if (length(input$simCont) == 0) {
        return
      }
  ...
  observe({
    ...
    isolate({
      ...
      toCpy <- mkfvsCmp(
        kwds = toCpy$kwds, kwdName = toCpy$kwdName,
  ...
  observe({
    ...
    isolate({
      ...
      if (length(input$selpaste) == 0) {
        return()
      }
      if (nchar(input$selpaste) == 0) {
        return()
      }
      ...
      if (class(topaste) != "fvsCmp") {
        return()
      }
      topaste <- mkfvsCmp(
        kwds = topaste$kwds, kwdName = topaste$kwdName,
  ...
  observe({
    ...
    isolate({
      ...
      if (substring(cmp$kwdName, 1, 10) == "Freeform: ") {
        return()
      }
  ...
  observe({
    cat(file = stderr(),
      "compTabSet, input$compTabSet=", input$compTabSet,
    ...
    switch(input$compTabSet,
      ...
      "Modifiers" = {
        if (length(globals$mmodsel) == 0) {
          globals$mmodsel <-
            mkcatsel(prms, "selectModelModifiers", globals)
        }
        updateSelectInput(
          session = session, inputId = "addModCats",
          choices = mkpair(globals$mmodsel), selected = 0
        )
        updateSelectInput(
          session = session, inputId = "addModCmps",
          choices = list()
        )
        output$cmdBuild <- output$cmdBuildDesc <- renderUI(NULL)
      },
      "Event Monitor" = {
        ...
        updateSelectInput(
          session = session, inputId = "addEvCmps",
      ...
      "Keywords" = {
        ...
        updateSelectInput(
          session = session, inputId = "addKeyExt",
          label = "Extensions", choices = globals$extnsel, selected = 0
        )
        updateSelectInput(
          session = session, inputId = "addKeyWds",
  ...
  observe({
    ...
    updateSelectInput(
      session = session, inputId = "addMgmtCmps", selected = 0,
  ...
  observe({
    ...
    updateSelectInput(
      session = session, inputId = "addModCmps", selected = 0,
  ...
  observe({
    ...
    updateSelectInput(
      session = session, inputId = "addKeyWds", selected = 0,
  ...
  observe({
    if (length(input$addEvCmps) &&
      nchar(input$addEvCmps)) {
      renderComponent("evn")
    }
  })

  renderComponent <- function(inCode = "default") {
    cat(file = stderr(),"renderComponent, inCode=", inCode, "\n")
    isolate({
      globals$currentEditCmp <- globals$NULLfvsCmp
      ...
      cat(file = stderr(),"globals$currentCmdPkey=", globals$currentCmdPkey, "\n")
      ...
      if (!is.null(funName)) {
        ...
        cat(file = stderr(),"function name=", globals$winBuildFunction, "\n")
        ans <- eval(parse(text = paste0(
          globals$winBuildFunction,
  ...
  observe({
    if (length(input$schedbox) == 0) {
      return()
    }
    cat(file = stderr(),"input$schedbox=", input$schedbox, "\n")
    if (input$schedbox == 1) {
      ...
      if (length(globals$currentCmdPkey)) {
        ...
        default <- getPstring(
          prms[[if (is.na(n)) globals$currentCmdPkey else n]],
  ...
  observe({
    ...
    if (length(input$condList) == 0) {
      return()
    }
    if (length(globals$toggleind) && input$schedbox == 1) {
      return()
    }
    cat(file = stderr(),"make condElts, input$condList=", input$condList, "\n")
    output$condElts <- renderUI(if (input$condList == "none") {
      NULL
    } else {
      cnpkey <- paste0("condition.", input$condList)
      idx <- match(cnpkey, names(prms))
      globals$currentCndPkey <- if (is.na(idx)) "0" else cnpkey
      ui <- if (globals$currentCndPkey == "0") {
        NULL
      } else {
        eltList <- mkeltList(prms[[globals$currentCndPkey]], prms,
          globals, globals$fvsRun,
          cndflag = TRUE
        )
  ...
  observe({
    # command Cancel
    if (length(input$cmdCancel) && input$cmdCancel == 0) {
      return()
    }
    closeCmp()
  })
  ...
  observe({
    # command Save in run
    if (length(input$cmdSaveInRun) && input$cmdSaveInRun == 0) {
      return()
    }
    isolate({
      if (identical(globals$currentEditCmp, globals$NULLfvsCmp) &&
        globals$currentCndPkey == "0" && globals$currentCmdPkey == "0") {
        return()
      }
      ...
      if (globals$currentCndPkey != "0") {
        ...
        cat(file = stderr(),"Save with if/then, kwds=", kwds, " reopn=", reopn, "\n")
        newcnd <- mkfvsCmp(
          uuid = uuidgen(), atag = "c", exten = "base",
      ...
      if (length(pkeys) == 0 && nchar(kwPname)) {
        ...
        ansFrm <- getPstring(pkeys, "answerForm", globals$activeVariants[1])
        if (is.null(ansFrm)) {
          cat(file = stderr(),"kwPname=", kwPname, "\n")
          kw <- kwPname[1]
        ...
        repeat        {
          ...
          instr <- if (length(globals$currentEditCmp$atag) &&
            ...
            input[[paste0("cnd.", pkey)]]
          } else {
            input[[pkey]]
          }
          if (fps == "scheduleBox" && !is.null(input$schedbox) &&
            input$schedbox == "1" &&
            !identical(newcnd, globals$NULLfvsCmp)) {
            newcnd <- NULL
          }
          reopn <- c(reopn, as.character(if (is.null(instr)) " " else instr))
          names(reopn)[fn] <- pkey
      ...
      cat(file = stderr(),"Save, kwds=", kwds, "\n")
      if (identical(globals$currentEditCmp, globals$NULLfvsCmp)) {
        ex <- if (length(kwPname) == 2) kwPname[2] else "base"
        cat(file = stderr(),"building component kwPname=", kwPname, "\n")
        newcmp <- mkfvsCmp(
          uuid = uuidgen(), atag = "k", kwds = kwds, exten = ex,
        ...
        if (is.null(grp) && is.null(std)) {
          ...
          if (is.null(grp)) {
            for (std in 1:length(globals$fvsRun$stands))
            {
        ...
        if (length(globals$schedBoxPkey) && input$schedbox == 3) {
          ...
          # adjust insert point.
          if (is.null(std)) {
            for (i in (cmp + 1):length(globals$fvsRun$grps[[grp]]$cmps))
            {
              if (i > length(globals$fvsRun$grps[[grp]]$cmps)) break
              if (globals$fvsRun$grps[[grp]]$cmps[[i]]$atag == sel) cmp <- i
            }
          } else {
            for (i in (cmp + 1):length(globals$fvsRun$stands[[std]]$cmps))
            {
        ...
        if (length(globals$schedBoxPkey) && input$schedbox == 1 &&
          length(input[[globals$schedBoxPkey]])) {
          globals$schedBoxYrLastUsed <-
        ...
        if (is.null(grp)) {
          globals$fvsRun$stands[[std]]$cmps <- if (is.null(cmp)) {
            append(globals$fvsRun$stands[[std]]$cmps, newcmp)
          } else {
            append(globals$fvsRun$stands[[std]]$cmps, newcmp, after = cmp)
          }
        } else {
          globals$fvsRun$grps[[grp]]$cmps <- if (is.null(cmp)) {
            append(globals$fvsRun$grps[[grp]]$cmps, newcmp)
          } else {
        ...
        globals$currentEditCmp$title <- input$cmdTitle
        cat(file = stderr(),"saving, kwds=", kwds, " title=", input$cmdTitle, " reopn=", reopn, "\n")
  ...
  ## time--start year
  observe({
    if (!length(input$simCont) || globals$fvsRun$startyr == input$startyr) {
      return()
    }
  ...
  ## time--end year
  observe({
    if (!length(input$simCont) || globals$fvsRun$endyr == input$endyr) {
      return()
    }
  ...
  ## time--cycle length
  observe({
    if (!length(input$simCont) || globals$fvsRun$cyclelen == input$cyclelen) {
      return()
    }
  ...
  observe({
    if (!length(input$simCont) || (length(globals$fvsRun$cycleat) &&
      length(input$cycleat) && globals$fvsRun$cycleat == input$cycleat)) {
      return()
    }
  ...
  observe({
    ...
    isolate({
      if (length(globals$fvsRun$stands) > 0) {
        ...
        progress$set(
          message = "Run preparation: ",
          detail = "Saving FVS Runs", value = 1
        )
        saveRun()
        cat(file = stderr(),"Nulling uiRunPlot at Save and Run\n")
        output$uiRunPlot <- output$uiErrorScan <- renderUI(NULL)
        globals$currentQuickPlot <- character(0)
        selChoices <- names(globals$FVS_Runs)
        names(selChoices) <- globals$FVS_Runs
        ...
        updateSelectInput(
          session = session, inputId = "bkgRuns",
        ...
        if (!file.exists(paste0(globals$fvsRun$uuid, ".key"))) {
          cat(file = stderr(),"keyword file was not created.\n")
          progress$set(
            message = "Error: Keyword file was not created.",
            detail = "", value = 3
          )
          Sys.sleep(3)
          progress$close()
        ...
        cat(file = stderr(),"runwaitback=", input$runwaitback, "\n")
        if (input$runwaitback != "Wait for run") {
          runScript <- paste0(globals$fvsRun$uuid, ".rscript")
          rs <- file(runScript, open = "wt")
          cat(file = stderr(),paste0('setwd("', getwd(), '")\n'), file = rs)
          cat(file = stderr(),"options(echo=TRUE)
                library(methods)
                library(RSQLite)
                ", file = rs)
          cat(file = stderr(),"pid = Sys.getpid()
", file = rs)
          ...
          if (globals$fvsRun$runScript != "fvsRun") {
            ...
            cat(file = stderr(),cmd, "\n", file = rs)
            cat(file = stderr(),"runOps = ", deparse(globals$fvsRun$uiCustomRunOps), "\n", file = rs)
          }
          cmd <- paste0('fvsSetCmdLine("--keywordfile=', globals$fvsRun$uuid, '.key")')
          cat(file = stderr(),cmd, "\n", file = rs)
          runCmd <- if (globals$fvsRun$runScript == "fvsRun") {
            "fvsRun()"
          } else {
          ...
          cat(file = stderr(),cmd, "\n", file = rs)
          cat(file = stderr(),'source("fvsRunUtilities.R")\n', file = rs)
          cmd <- paste0(
            'dbDrv = dbDriver("SQLite")\n',
          ...
          cat(file = stderr(),cmd, "\n", file = rs)
          cmd <- paste0(
            'cat (pid,"Adding results to output database; title=",title,"\n",file="', pidfile,
            '")'
          )
          cat(file = stderr(),cmd, "\n", file = rs)
          cmd <- paste0('addNewRun2DB("', globals$fvsRun$uuid, '",dbOcon)')
          cat(file = stderr(),cmd, "\n", file = rs)
          cat(file = stderr(),"dbDisconnect(dbOcon)\n", file = rs)
          cmd <- paste0("unlink('", pidfile, "')")
          cat(file = stderr(),cmd, "\n", file = rs)
          progress$set(
            message = "Run starting in background",
            detail = "", value = 4
          )
          ...
          cmd <- paste0(
            rscript, " --no-restore --no-save --no-init-file ", runScript,
            " > ", runScript, ".Rout"
          )
        ...
        cmd <- paste0(
          "clusterEvalQ(fvschild,for (rf in dir('", locrFVSDir,
          "')) source(paste0('", locrFVSDir, "','/',rf)))"
        )
        cat(file = stderr(),"load rFVS cmd=", cmd, "\n")
        rtn <- try(eval(parse(text = cmd)))
        if (class(rtn) == "try-error") {
          return()
        }
        cmd <- paste0(
          "clusterEvalQ(fvschild,fvsLoad('",
          globals$fvsRun$FVSpgm, "',bin='", locbinDir, "'))"
        )
        cat(file = stderr(),"load FVSpgm cmd=", cmd, "\n")
        rtn <- try(eval(parse(text = cmd)))
        if (class(rtn) == "try-error") {
          return()
        }
        ...
        if (globals$fvsRun$runScript != "fvsRun") {
          cmd <- paste0(
            "clusterEvalQ(fvschild,",
            "source('customRun_", globals$fvsRun$runScript, ".R'))"
          )
          cat(file = stderr(),"run script load cmd=", cmd, "\n")
          rtn <- try(eval(parse(text = cmd)))
          if (class(rtn) == "try-error") {
            return()
          }
          runOps <<- if (is.null(globals$fvsRun$uiCustomRunOps)) {
            list()
          } else {
        ...
        cat(file = stderr(),"load run cmd=", cmd, "\n")
        rtn <- try(eval(parse(text = cmd)))
        if (class(rtn) == "try-error") {
          return()
        }
        ...
        on.exit({
          progress$close()
          cat(file = stderr(),"exiting, stop fvschild\n")
          try(stopCluster(fvschild))
        })

        cat(file = stderr(),"at for start\n")
        allSum <- list()
        for (i in 1:length(globals$fvsRun$stands))
        {
          ...
          rtn <- if (globals$fvsRun$runScript != "fvsRun") {
            ...
            cat(file = stderr(),"custom run cmd=", cmd, "\n")
            try(eval(parse(text = cmd)))
          } else {
            cat(file = stderr(),"running normal run cmd\n")
            try(clusterEvalQ(fvschild, fvsRun()))
          }
          if (class(rtn) == "try-error") {
            cat(file = stderr(),"run try error\n")
            break
          }
          cat(file = stderr(),"rtn class for stand i=", i, " is ", class(rtn), "\n")
        ...
        progress$set(
          message = if (length(allSum) == length(globals$fvsRun$stands)) {
            "FVS finished"
          } else {
            "FVS run failed"
          }, detail = "",
        ...
        Sys.sleep(.1)
        cat(file = stderr(),"length(allSum)=", length(allSum), "\n")
        if (length(allSum) == 0) {
          Sys.sleep(.4)
          return()
        }
        progress$set(
          message = "FVS finished",
        ...
        cat(file = stderr(),"addNewRun2DB res=", res, "\n")
        unlink(paste0(globals$fvsRun$uuid, ".db"))
        X <- vector("numeric", 0)
        ...
        for (i in 1:length(allSum))
        {
          ...
          Legend <- c(Legend, c(
            rep("TCuFt", nrow(allSum[[i]])),
            rep("TPrdTCuFt", nrow(allSum[[i]]))
          ))
        }
        toplot <- data.frame(
          X = X, hfacet = as.factor(hfacet), Y = Y,
        ...
        output$runPlot <- renderImage(list(
          src = "quick.png", width = (width + 1) * 144,
          height = (height + 1) * 144
        ), deleteFile = TRUE)
        cat(file = stderr(),"setting currentQuickPlot, input$runSel=", input$runSel, "\n")
  ...
  output$dlFVSOutxlsx <- downloadHandler(
    ...
    content = function(tf = paste0(tempfile(), ".xlsx")) {
      ...
      if (!("FVS_Cases" %in% tabs)) {
        return()
      }
      cases <- dbGetQuery(dbGlb$dbOcon, paste0(
        "select CaseID from FVS_Cases ",
        "where KeywordFile = '", globals$fvsRun$uuid, "';"
      ))
      if (nrow(cases) == 0) {
        return()
      }
      cat(file = stderr(),"download run as xlsx, ncases=", nrow(cases), "\n")
      ...
      for (tab in tabs)
      {
        qry <- if (cmpYes && substr(tab, 1, 3) == "Cmp") {
          paste0("select * from ", tab, " limit 1048576;")
        } else {
          paste0(
            "select * from ", tab, " where ", tab, ".CaseID in",
        ...
        if (nrow(dat) == 0) next
        out[[tab]] <- dat
        cat(file = stderr(),"qry=", qry, " class(dat)=", class(dat), "\n")
      }
    ...
    content = function(tf = tempfile()) {
      if (isolate(input$dlRDType) == ".csv") {
        if (nrow(fvsOutData$render) > 0) {
          write.csv(fvsOutData$render, file = tf, row.names = FALSE)
        } else {
          cat(file = stderr(),file = tf, '"No data"\n')
        }
      } else {
        if (nrow(fvsOutData$render) > 0) {
          if (nrow(fvsOutData$render) > 1048576) {
            write.xlsx(fvsOutData$render[1:1048576, ], file = tf, colNames = TRUE)
          } else {
  ...
  output$dlPrjBackup <- downloadHandler(
    ...
    content = function(tf = tempfile()) {
      ...
      if (file.exists(sfile)) {
        ...
        cat(file = stderr(),file = tf, "Backup does not exist.\n")
      }
    }, contentType = "zip"
  )
  ## DownLoad
  output$dlFVSRunout <- downloadHandler(
    ...
    content = function(tf = tempfile()) {
      ...
      if (file.exists(sfile)) {
        ...
        cat(file = stderr(),file = tf, "Output not yet created.\n")
      }
    }, contentType = "text"
  )
  ## Download keywords
  output$dlFVSRunkey <- downloadHandler(
    ...
    content = function(tf = tempfile()) {
      ...
      if (file.exists(sfile)) {
        ...
        cat(file = stderr(),file = tf, "Keywords not yet created.\n")
      }
    }, contentType = "text"
  )
  ...
  output$dlFVSRunZip <- downloadHandler(
    filename = "FVSData.zip",
    content = function(tf = tempfile()) {
      tempDir <- paste0(dirname(tf), "/tozip")
      if (dir.exists(tempDir)) {
        lapply(paste0(tempDir, "/", dir(tempDir)), unlink)
      } else {
        dir.create(tempDir)
      }
        ...
        switch(ele,
          outdb = {
            ...
            if (file.exists(from)) {
              ...
              cat(file = stderr(),file = to, "Output database does not exist.\n")
            }
          },
          key = {
            ...
            if (file.exists(from)) {
              ...
              cat(file = stderr(),file = to, "Keyword file not yet created.\n")
            }
          },
          out = {
            ...
            if (file.exists(from)) {
              ...
              cat(file = stderr(),file = to, "Output not yet created.\n")
            }
          },
          subdir = {
            from <- input$runSel
            if (dir.exists(from)) {
              ...
              cat(file = stderr(),file = paste0(tempDir, "/emptySVS.txt"), "No SVS files exist.\n")
            }
          },
          FVS_Data = file.copy(
            from = "FVS_Data.db",
          ...
          FVS_Runs = {
            ...
            cat(file = stderr(),file = rdm, append = TRUE, "Run UUIDs and titles as of:", date(), "\n")
            for (kn in names(globals$FVS_Runs))
            {
              cat(file = stderr(),file = rdm, append = TRUE, "UUID:", kn, " Title:", globals$FVS_Runs[[kn]], "\n")
              fn <- paste0(kn, ".RData")
  ...
  ## kcpSel
  observe({
    if (length(input$kcpSel) == 0) {
      return()
    }
    cat(file = stderr(),"kcpSel called, input$kcpSel=", input$kcpSel, "\n")
    if (is.null(input$kcpSel)) {
      ...
      updateTextInput(
        session = session, inputId = "kcpTitle",
        value = names(globals$customCmps)[sel]
      )
      updateTextInput(
        session = session, inputId = "kcpEdit",
  ...
  observe({
    if (length(input$kcpSaveCmps) && input$kcpSaveCmps > 0) {
      isolate({
        cat(file = stderr(),
          "kcpSaveCmps called, kcpTitle=", input$kcpTitle, " isnull=",
        ...
        if (length(grep("^--> Kwd", names(globals$kcpAppendConts[length(globals$kcpAppendConts)])))) {
          updateTextInput(
            session = session, inputId = "kcpEdit", value =
        ...
        updateSelectInput(
          session = session, inputId = "kcpSel",
  ...
  observe({
    if (length(input$kcpSaveInRun) && input$kcpSaveInRun > 0) {
      isolate({
        cat(file = stderr(),"kcpSaveInRun\n")
        if (nchar(input$kcpTitle) == 0) {
          newTit <- paste0("Freeform: Component ", length(globals$customCmps) + 1)
          updateTextInput(session = session, inputId = "kcpTitle", value = newTit)
        } else {
          newTit <- paste0("Addfile: ", trim(input$kcpTitle))
        }
        newcmp <- mkfvsCmp(
          uuid = uuidgen(), atag = "k", kwds = input$kcpEdit, exten = "base",
  ...
  observe({
    if (length(input$kcpDelete) && input$kcpDelete > 0) {
      isolate({
        cat(file = stderr(),"kcpDelete, input$kcpSel=", input$kcpSel, "\n")
  ...
  observe({
    if (length(input$kcpNew) && input$kcpNew > 0) {
      ...
      cat(file = stderr(),"kcpNew called, input$kcpNew=", input$kcpNew, "\n")
    }
  })

  ## kcpUpload
  observe({
    if (is.null(input$kcpUpload)) {
      return()
    }
    ...
    isolate({
      ...
      if (is.null(input$kcpTitle) || nchar(input$kcpTitle) == 0) {
        addnl <- FALSE
        updateTextInput(
          session = session, inputId = "kcpTitle", value =
  ...
  observe({
    if (input$topPan == "SVS3d(alpha)") {
      ...
      updateSelectInput(
        session = session, inputId = "SVSRunList1",
        choices = runChoices, selected = 0
      )
      updateSelectInput(
        session = session, inputId = "SVSRunList2",
        choices = runChoices, selected = 0
      )
      updateSelectInput(
        session = session, inputId = "SVSImgList1", choices = list(),
        selected = 0
      )
      updateSelectInput(
        session = session, inputId = "SVSImgList2", choices = list(),
        selected = 0
      )
  ...
  observe({
    if (length(input$SVSRunList1)) {
      cat(file = stderr(),"SVS3d input$SVSRunList1=", input$SVSRunList1, "\n")
      ...
      updateSelectInput(
        session = session, inputId = "SVSImgList1", choices = choices,
        selected = 0
      )
      output$SVSImg1 <- renderRglwidget(NULL)
    }
  ...
  observe({
    if (length(input$SVSRunList2)) {
      cat(file = stderr(),"SVS3d input$SVSRunList2=", input$SVSRunList2, "\n")
      ...
      names(choices) <- index[, 1]
      updateSelectInput(
        session = session, inputId = "SVSImgList2", choices = choices,
        selected = 0
      )
      output$SVSImg2 <- renderRglwidget(NULL)
    }
  ...
  renderSVSImage <- function(id, imgfile) {
    ...
    if (length(rcirc)) {
      ...
      cat(file = stderr(),"args=", args, "\n")
      circle3D(x0 = args[1], y0 = args[2], r = args[3], alpha = 0.5)
    } else {
      return()
    } # what if it is not a circle???????
    fireline <- grep("^#FIRE_LINE", svs)
    cat(file = stderr(),"length(fireline)=", length(fireline), "\n")
    if (length(fireline)) {
      ...
      if (sum(k) > 1) {
        ...
        fls <- t(apply(fls, 1, function(x) c(
            x[1] - x[3], x[2], 0, x[1], x[2], x[3] * 3,
            x[1] + x[3], x[2], 0
          )))
        verts <- NULL
    ...
    cat(file = stderr(),"N flames=", length(flames), "\n")
    if (length(flames)) {
      calls <- calls + 1
    ...
    cat(file = stderr(),"Residual length of svs=", length(svs), "\n")
    drawnTrees <- list()
    trees <- list()
    for (line in svs)
    {
      ...
      names(tree) <- c(
        "TrNum", "TrCl", "CrCl", "Stus", "DBH", "Ht", "Lang",
  ...
  observe({
    if (length(input$SVSImgList1)) {
      cat(file = stderr(),"SVS3d input$SVSImgList1=", input$SVSImgList1, "\n")
      if (!file.exists(input$SVSImgList1)) {
        return()
      }
  ...
  observe({
    if (length(input$SVSImgList2)) {
      cat(file = stderr(),"SVS3d input$SVSImgList2=", input$SVSImgList2, "\n")
      if (!file.exists(input$SVSImgList2)) {
        return()
      }
  ...
  observe({
    if (input$topPan == "Maps(alpha)") {
      cat(file = stderr(),"Maps hit\n")
      require(rgdal)
      allRuns <- names(globals$FVS_Runs)
      names(allRuns) <- globals$FVS_Runs
      updateSelectInput(
        session = session, inputId = "mapDsRunList",
        choices = allRuns, selected = 0
      )
      updateSelectInput(
        session = session, inputId = "mapDsTable", choices = list(),
        selected = 0
      )
      updateSelectInput(
        session = session, inputId = "mapDsVar", choices = list(),
        selected = 0
      )
      updateSelectInput(
        session = session, inputId = "MapYear", choices = list(),
        selected = 0
      )
  ...
  observe({
    if (length(input$mapDsRunList)) {
      cat(file = stderr(),"mapDsRunList input$mapDsRunList=", input$mapDsRunList, "\n")
      cases <- dbGetQuery(
        dbGlb$dbOcon,
        paste0(
          "select CaseID,StandID from FVS_Cases where KeywordFile = '",
          input$mapDsRunList, "'"
        )
      )
      ...
      for (tab in tabs)
      {
        ...
        cnt <- try(dbGetQuery(dbGlb$dbOcon, paste0(
      ...
      updateSelectInput(
        session = session, inputId = "mapDsTable", choices = tables,
        selected = 0
      )
      updateSelectInput(
        session = session, inputId = "mapDsVar", choices = list(),
  ...
  observe({
    if (length(input$mapDsTable)) {
      cat(file = stderr(),"mapDsRunList input$mapDsTable=", input$mapDsTable, "\n")
      vars <- setdiff(
        dbListFields(dbGlb$dbOcon, input$mapDsTable),
        c("CaseID", "StandID", "Year")
      )
      vars <- as.list(vars)
      names(vars) <- vars
      updateSelectInput(
        session = session, inputId = "mapDsVar", choices = vars,
        selected = 0
      )
      output$leafletMap <- renderLeaflet(NULL)
    }
  ...
  observe({
    if (length(input$mapDsVar)) {
      require(rgdal)
      cat(file = stderr(),
        "mapDsRunList input$mapDsTable=", isolate(input$mapDsTable),
        " input$mapDsVar=", input$mapDsVar, " input$mapDsType=", input$mapDsType, "\n"
      )
      if (!exists("SpatialData", envir = dbGlb, inherit = FALSE) &&
        file.exists("SpatialData.RData")) {
        load("SpatialData.RData", envir = dbGlb)
      }
      if (!exists("SpatialData", envir = dbGlb, inherit = FALSE)) {
        cat(file = stderr(),"mapDsRunList trying to use StandInit data\n")
        inInit <- getTableName(dbGlb$dbIcon, "FVS_StandInit")
        dbGlb$SpatialData <- try(dbGetQuery(
          dbGlb$dbIcon,
        ...
        if (class(dbGlb$SpatialData) != "try-error") {
          ...
          cat(file = stderr(),"mapDsRunList idxLng=", idxLng, " idxLat=", idxLat, " names=", names(dbGlb$SpatialData), "\n")
          ...
          if (is.null(dbGlb$SpatialData) || nrow(dbGlb$SpatialData) == 0) {
            cat(file = stderr(),"mapDsRunList trying PlotInit\n")
            inInit <- getTableName(dbGlb$dbIcon, "FVS_PlotInit")
            dbGlb$SpatialData <- try(dbGetQuery(
              dbGlb$dbIcon,
              paste0(
                "select Stand_ID,avg(Latitude) as Latitude, ",
        ...
        if (class(dbGlb$SpatialData) != "data.frame") {
          ...
          cat(file = stderr(),"mapDsRunList names(dbGlb$SpatialData)=", names(dbGlb$SpatialData), " class(dbGlb$SpatialData)=", class(dbGlb$SpatialData), "\n")
          idxLng <- grep("Longitude", names(dbGlb$SpatialData), ignore.case = TRUE)
          idxLat <- grep("Latitude", names(dbGlb$SpatialData), ignore.case = TRUE)
          idxID <- grep("Stand_ID", names(dbGlb$SpatialData), ignore.case = TRUE)
          cat(file = stderr()," idxLng=", idxLng, " idxLat=", idxLat, " idxID=", idxID, "\n")
      ...
      cat(file = stderr(),"matchVar=", matchVar, "\n")
      dispData <- dbGetQuery(dbGlb$dbOcon, paste0(
        "select * from ",
      ...
      extra <- NULL
      if (any(table(dispData[, c("StandID", "Year")]) > 1)) {
        for (var in keys)
        {
          if (class(dispData[, var]) == "character") {
            extra <- c(extra, var)
      ...
      if (class(dispData[, input$mapDsVar]) == "numeric") {
        dispData[, input$mapDsVar] <-
        ...
        if (input$mapDsType == "table" || any(is.na(as.numeric(tab[, input$mapDsVar])))) {
          HTML(paste0('<p style="LINE-HEIGHT:1">StandID=', sid, "<br>",
            paste(names(tab), collapse = " "), "<br>", paste0(apply(tab, 1, function(x)
              paste0(paste0(x, collapse = " "))), collapse = "<br>"), "</p>",
            collapse = ""
          ))
        } else {
          tab <- subset(dispData, StandID == sid)[, -1]
          ...
          cat(file = stderr(),"pfile=", pfile, " nrow=", nrow(tab), " sid=", sid, "\n")
          png(file = pfile, height = 1.7, width = 2.3, units = "in", res = 100, bg = "transparent")
          p <- ggplot(tab, if (!is.null(extra)) {
            aes_string(x = "Year", y = input$mapDsVar, linetype = extra)
          } else {
          ...
          HTML(paste0(
            '<img src="', pfile, "?", as.character(as.numeric(Sys.time())),
      ...
      map <- leaflet(data = polys) %>%
        ...
        addTiles(
          urlTemplate =
            paste0(
              "https://mts1.google.com/vt/lyrs=", input$mapDsProvider,
              "&hl=en&src=app&x={x}&y={y}&z={z}&s=G"
            ), attribution = "Google"
        )
      ...
      if (class(polys) == "SpatialPointsDataFrame") {
        ...
        map <- map %>% addPolygons(
          color = "red", weight = 2, smoothFactor = 0.1,
  ...
  observe({
    if (input$topPan == "Tools") {
      ...
      if (exists("fvsBinDir") && !is.null(fvsBinDir) && file.exists(fvsBinDir) &&
        ...
        cat(file = stderr(),"avalFVSp=", avalFVSp, "\n")
      } else {
        pgmFlip <- list("Refresh not supported on this system.")
        haveFVSp <- NULL
      }
      updateSelectInput(
        session = session, inputId = "FVSprograms",
      ...
      updateSelectInput(
        session = session, inputId = "pickBackup",
  ...
  ## FVSRefresh
  observe({
    if (input$FVSRefresh == 0) {
      return()
    }
    cat(file = stderr(),"FVSRefresh\n")
    isolate({
      if (length(input$FVSprograms) == 0) {
        return()
      }
      shlibsufx <- if (.Platform$OS.type == "windows") ".dll" else ".so"
      i <- 0
      if (exists("fvsBinDir") && !is.null(fvsBinDir) && file.exists(fvsBinDir)) {
        for (pgm in input$FVSprograms)
        {
          ...
          cat(file = stderr(),"copy frm=", frm, " tto=", tto, "\n")
  ...
  observe({
    if (input$deleteRunDlgBtn > 0) {
      isolate({
        cat(file = stderr(),
          "delete run", globals$fvsRun$title, " uuid=", globals$fvsRun$uuid,
  ...
  observe({
    if (input$deleteAllOutputsDlgBtn == 0) {
      return()
    }
    isolate({
      cat(file = stderr(),"delete all outputs\n")
  ...
  observe({
    if (input$deleteAllRunsDlgBtn == 0) {
      return()
    }
    isolate({
      cat(file = stderr(),"delete all runs and outputs\n")
  ...
  observe({
    if (length(input$interfaceRefreshDlgBtn) &&
      ...
      cat(file = stderr(),
        "input$interfaceRefreshDlgBtn=", input$interfaceRefreshDlgBtn,
      ...
      if (exists("fvsOnlineDir") && exists("FVSOnlineNeeded")) {
        ...
        cat(file = stderr(),"ffdir=", ffdir, "\n")
        if (file.exists(ffdir)) {
          lapply(FVSOnlineNeeded, function(x, fd) {
            frm <- paste0(fd, "/", x)
            cat(file = stderr(),"Refresh interface file=", frm, "\n")
  ...
  observe({
    if (length(input$restorePrjBackupDlgBtn) &&
      ...
      isolate({
        ...
        cat(file = stderr(),"restorePrjBackupDlgBtn fvsWorkBackup=", fvsWorkBackup, "\n")
  ...
  ## rpBldDwnLd
  output$rpBldDwnLd <- downloadHandler(
    filename = "FVSReport.docx",
    content = function(tf = tempfile()) generateReport(tf)
  )
  ## rpTableAdd
  observe({
    if (input$rpTableAdd == 0) {
      return()
    }
  ...
  xlsx2html <- function(tab = NULL, xlsxfile = "databaseDescription.xlsx") {
    if (!file.exists(xlsxfile) || is.null(tab)) {
      return(NULL)
    }
  ...
  observe({
    ...
    cat(file = stderr(),"tabDescSel, tab=", tab, "\n")
    html <- NULL
  ...
  observe({
    if (input$topPan == "Import Data") {
      updateTabsetPanel(
        session = session, inputId = "inputDBPan",
  ...
  observe({
    if (input$inputDBPan == "Replace existing database") {
      cat(file = stderr(),"Replace existing database\n")
      output$replaceActionMsg <- renderText("")
    }
  })
  ...
  ## Upload new database
  observe({
    if (is.null(input$uploadNewDB)) {
      return()
    }
    fext <- tools::file_ext(basename(input$uploadNewDB$name))
    cat(file = stderr(),"fext=", fext, "\n")
    session$sendCustomMessage(
      type = "jsCode",
    ...
    if (fext == "zip") {
      ...
      fname <- basename(input$uploadNewDB$datapath)
    }
    cat(file = stderr(),"fext=", fext, " fname=", fname, " fdir=", fdir, "\n")
    ...
    if (fext %in% c("accdb", "mdb")) {
      ...
      cat(file = stderr(),"curDir=", curDir, " input dir=", getwd(), "\n")
      cmd <- if (.Platform$OS.type == "windows") {
        shQuote(paste0(
          'java -jar "', curDir, '/access2csv.jar" "',
          fname, '" --schema'
        ), type = "cmd2")
      } else {
        paste0(
      ...
      for (i in 1:length(schema))
      {
        ...
        if (substring(schema[i], 1, 12) == "CREATE TABLE") {
          ...
          cknt <- NULL
          if (length(grep("FVS_STANDINIT", tblName, ignore.case = TRUE))) {
            cknt <- standNT
          } else if (length(grep("FVS_STANDINIT_COND", tblName, ignore.case = TRUE))) {
            cknt <- standNT
          } else if (length(grep("FVS_STANDINIT_PLOT", tblName, ignore.case = TRUE))) {
            cknt <- standNT
          } else if (length(grep("FVS_TREEINIT", tblName, ignore.case = TRUE))) {
            cknt <- treeNT
          } else if (length(grep("FVS_TREEINIT_COND", tblName, ignore.case = TRUE))) {
            cknt <- treeNT
          } else if (length(grep("FVS_TREEINIT_PLOT", tblName, ignore.case = TRUE))) {
            cknt <- treeNT
          } else if (length(grep("FVS_PLOTINIT", tblName, ignore.case = TRUE))) {
            cknt <- plotNT
      ...
      cat(file = stderr(),paste0(schema, "\n"), file = "schema")
      progress$set(message = "Extract data", value = 3)
      cmd <- if (.Platform$OS.type == "windows") {
        shQuote(paste0('java -jar "', curDir, '/access2csv.jar" "', fname), type = "cmd2")
      } else {
      ...
      for (s in schema)
      {
        ...
        cat(file = stderr(),"s=", s, " cmd=", cmd, "; ")
        if (.Platform$OS.type == "windows") shell(cmd) else system(cmd)
        cat(file = stderr(),"cmd done.\n")
      }
      ...
      i <- 0
      normNames <- c(
        "FVS_GroupAddFilesAndKeywords", "FVS_PlotInit",
      ...
      {
        i <- i + 1
        cat(file = stderr(),"sheet = ", sheet, " i=", i, "\n")
    ...
    for (tab in tabs)
    {
      cat(file = stderr(),"loaded table=", tab, "\n")
      nn <- sub("NRIS_", "", tab)
      if (nchar(nn) && nn != tab) dbExecute(dbo, paste0("alter table ", tab, " rename to ", nn))
    }
    tabs <- dbGetQuery(dbo, "select name from sqlite_master where type='table';")[, 1]
    rowCnts <- unlist(lapply(tabs, function(x) dbGetQuery(
        dbo,
  ...
  ## installNewDB
  observe({
    if (input$installNewDB == 0) {
      return()
    }
    if (is.null(dbGlb$newFVSData)) {
      return()
    }
    dbDisconnect(dbGlb$dbIcon)
    ...
    for (tb in tabs)
    {
      ...
      if (tolower(tb) == "fvs_climattr") {
        ...
        if (class(try) != "try-error") {
          ...
          cat(file = stderr(),"index creation, qry=", qry, "\n")
  ...
  observe({
    ...
    while (TRUE) {
      ...
      cat(file = stderr(),"try to get exclusive lock on input database, trycnt=", trycnt, "\n")
    ...
    for (tab in newtabs)
    {
      ...
      if (tolower(tab) %in% c("fvs_standinit", "fvs_treeInit", "fvs_plotInit")) {
        dbExecute(dbGlb$dbIcon, paste0(
          "delete from ", tab, " where Stand_ID in ",
          "(select Stand_ID from addnew.", tab, ")"
        ))
      } else if (tolower(tab) == "fvs_groupaddfilesandkeywords") {
        dbExecute(dbGlb$dbIcon, paste0(
          "delete from ", tab, " where Groups in ",
      ...
      if (length(missingIndx) && !any(is.na(missingIndx))) {
        ...
        {
          qry <- paste0(
            "alter table ", tab, " add column ", newTdef$name[i],
            " ", newTdef$type[i], ";"
          )
          cat(file = stderr(),"homogenize qry=", qry, "\n")
          dbExecute(dbGlb$dbIcon, qry)
        }
      }
      alln <- paste0(newTdef$name, collapse = ",")
      qry <- paste0(
        "insert into ", tab, " (", alln, ") select ", alln,
        " from addnew.", tab, ";"
      )
      cat(file = stderr(),"homogenize qry=", qry, "\n")
      dbExecute(dbGlb$dbIcon, qry)
  ...
  observe({
    if (input$inputDBPan == "Upload and add new rows to existing tables (.csv)") {
      cat(file = stderr(),"Upload new rows\n")
      ...
      if (length(tbs)) {
        ...
        updateSelectInput(
          session = session, inputId = "uploadSelDBtabs", choices = tbs,
          selected = tbs[idx]
        )
      } else {
        updateSelectInput(
          session = session, inputId = "uploadSelDBtabs",
          choices = list()
        )
      }
      output$replaceActionMsg <- renderText(if (length(tbs)) {
        ""
      } else {
  ...
  ## uploadStdTree
  observe({
    if (is.null(input$uploadStdTree)) {
      return()
    }
    isolate({
      ...
      cols <- na.omit(charmatch(
        tolower(colnames(indat)),
        tolower(names(dbGlb$tbsCTypes[[input$uploadSelDBtabs]]))
      ))
      if (length(cols) == 0) {
        output$uploadActionMsg <- renderText(paste0(
          "No columns match what is defined for ",
      ...
      cat(file = stderr(),"addCols=", addCols, "\n")
      if (length(addCols)) {
        for (icol in addCols)
        {
          ...
          qry <- paste0(
            "alter table ", input$uploadSelDBtabs, " add column ",
            newVar, " ", dtyp, ";"
          )
          cat(file = stderr(),"add column qry=", qry, "\n")
      ...
      if (!is.null(req) && !all(req %in% names(types))) {
        output$uploadActionMsg <- renderText(paste0(
          "Required columns were missing for ",
          input$uploadSelDBtabs, ", no data loaded."
        ))
        session$sendCustomMessage(type = "resetFileInputHandler", "uploadStdTree")
        return()
      ...
      for (i in 1:nrow(indat))
      {
        ...
        if (ncol(row) == 0) next
        qry <- paste0(
          "insert into ", input$uploadSelDBtabs, " (",
          paste0(colnames(row), collapse = ","),
          ") values (", paste0(row[1, ], collapse = ","), ");"
        )
        cat(file = stderr(),"insert qry=", qry, "\n")
      ...
      if (err) {
        ...
        cat(file = stderr(),"insertCount=", insertCount, "\n")
        if (insertCount) {
          dbCommit(dbGlb$dbIcon)
          output$uploadActionMsg <- renderText(paste0(
            insertCount, " row(s) inserted into ",
      ...
      if (!is.null(keyCol)) {
        ...
        if (keyCol != "Groups") {
          ...
          if (dbGlb$rowSelOn && length(dbGlb$sids)) {
            updateSelectInput(
              session = session, inputId = "rowSelector",
              choices = as.list(dbGlb$sids), selected = unique(indat[, "Stand_ID"])
            )
          } else {
          ...
          qry <- if (length(input$rowSelector)) {
            paste0(
              qry, " where Stand_ID in (",
              paste0("'", input$rowSelector, "'", collapse = ","), ");"
            )
          } else {
            paste0(qry, ";")
          }
          ...
          if (nrow(dbGlb$tbl) == 0) {
            ...
            output$tbl <- renderRHandsontable(rhandsontable(
              dbGlb$tbl[dbGlb$rows[1]:dbGlb$rows[2],
                union(c("Delete"), input$selectdbvars),
                drop = FALSE
              ],
  ...
  ## climateFVSUpload
  observe({
    if (is.null(input$climateFVSUpload)) {
      return()
    }
    ...
    if (input$climateFVSUpload$type == "application/zip") {
      cmd <- paste0(
        "unzip -l -p ", input$climateFVSUpload$datapath,
    ...
    if (!file.exists("FVSClimAttrs.csv")) {
      cat(file = stderr(),"no FVSClimAttrs.csv file\n")
    ...
    cat(file = stderr(),"processing FVSClimAttrs.csv\n")
    progress$set(message = "Loading data set (big files take a while)", value = 2)
    climd <- read.csv("FVSClimAttrs.csv", nrows = 1)
    climd <- read.csv("FVSClimAttrs.csv", colClasses = c(
      rep("character", 2),
      "integer", rep("numeric", ncol(climd) - 3)
    ), as.is = TRUE)
    ...
    if (!("FVS_ClimAttrs" %in% climTab)) {
      cat(file = stderr(),"no current FVS_ClimAttrs\n")
    ...
    cat(file = stderr(),"current FVS_ClimAttrs\n")
    ...
    results <- apply(distinct, 1, function(x, dbIcon) {
      dbExecute(dbIcon, paste0(
        'delete from FVS_ClimAttrs where Stand_ID = "',
    ...
    if (nrow(oldAttrs) == 0) {
      cat(file = stderr(),"simple copy from new, all rows were deleted\n")
      ...
      if (identical(colnames(oldAttrs), colnames(newAttrs))) {
        cat(file = stderr(),"simple insert from new, all cols are identical\n")
        dbExecute(dbGlb$dbIcon, "insert into FVS_ClimAttrs select * from new.FVS_ClimAttrs")
      } else {
        cat(file = stderr(),"need to match columns, cols are not identical\n")
        ...
        cat(file = stderr(),"length(newmiss)=", length(newmiss), " selnew=", selnew, "\n")
        if (length(newmiss) > 0) {
          dbBegin(dbGlb$dbIcon)
          for (mis in newmiss) {
            dbExecute(
        ...
        cat(file = stderr(),"length(oldmiss)=", length(oldmiss), "\n")
        if (length(oldmiss) > 0) {
  ...
  observe({
    if (input$inputDBPan == "View and edit existing tables") {
      cat(file = stderr(),"dataEditor View and edit existing tables\n")
      tbs <- dbGetQuery(dbGlb$dbIcon, "select name from sqlite_master where type='table';")[, 1]
      dbGlb$tbsCTypes <- lapply(tbs, function(x, dbIcon) {
        tb <- dbGetQuery(dbIcon, paste0("PRAGMA table_info('", x, "')"))
        tbtypes <- toupper(tb[, "type"])
      ...
      if (length(idx) == 0) idx <- 1
      updateSelectInput(
        session = session, inputId = "editSelDBtabs", choices = tbs,
        selected = tbs[idx]
      )
    }
  })
  ...
  observe({
    cat(file = stderr(),
      "editSelDBtabs, input$editSelDBtabs=", input$editSelDBtabs,
    ...
    if (length(input$editSelDBtabs)) {
      ...
      if (length(grep("Stand_ID", dbGlb$tblCols, ignore.case = TRUE))) {
        ...
        if (length(dbGlb$sids) > 0) {
          if (dbGlb$rowSelOn) {
            updateSelectInput(
              session = session,
              inputId = "rowSelector", choices = dbGlb$sids
            )
          } else {
            output$stdSel <- mkStdSel(dbGlb)
          }
      ...
      updateSelectInput(
        session = session, inputId = "editSelDBvars",
  ...
  observe({
    if (length(input$editSelDBvars)) {
      cat(file = stderr(),"editSelDBvars, input$editSelDBvars=", input$editSelDBvars, "\n")
      ...
      switch(input$mode,
        "New rows" = {
          ...
          tbl <- as.data.frame(matrix("",
            ncol = length(input$editSelDBvars),
            nrow = dbGlb$disprows
          ))
          ...
          if (nrow(dbGlb$tbl) == 0) {
            ...
            output$tbl <- renderRHandsontable(
              rhandsontable(dbGlb$tbl[1:min(nrow(dbGlb$tbl), dbGlb$disprows),
                union(c("Delete"), input$editSelDBvars),
                drop = FALSE
              ],
              readOnly = FALSE, useTypes = TRUE, contextMenu = FALSE
              )
            ...
            output$rowRng <- renderText(paste0(
              dbGlb$rows[1], " to ",
  ...
  observe({
    if (input$commitChanges > 0) {
      isolate({
        cat(file = stderr(),"commitChanges, mode=", input$mode, "len tbl=", length(input$tbl), "\n")
          ...
          Edit = {
            ...
            if (nrows) {
              for (rn in 1:nrows)
              {
                ...
                if (row["Delete"] == "TRUE") {
                  qry <- paste0(
                    "delete from ", dbGlb$tblName, " where _ROWID_ = ",
                    id
                  )
                  cat(file = stderr(),"edit del, qry=", qry, "\n")
                  res <- try(dbExecute(dbGlb$dbIcon, qry))
                  if (class(res) == "try-error") {
                    err <- TRUE
                    break
                  ...
                  update[update == ""] <- "NULL"
                  qry <- paste0(
                    "update ", dbGlb$tblName, " set ",
                    paste(paste0(names(update), " = ", update), collapse = ", "),
                    " where _ROWID_ = ", id
                  )
                  cat(file = stderr(),"edit upd, qry=", qry, "\n")
                  res <- try(dbExecute(dbGlb$dbIcon, qry))
                  if (class(res) == "try-error") {
                    err <- TRUE
                    break
            ...
            fixEmptyTable(dbGlb)
            cat(file = stderr(),
              "after commit, is.null(dbGlb$sids)=", is.null(dbGlb$sids),
  ...
  observe(if (input$clearTable > 0) {
    cat(file = stderr(),"clearTable, tbl=", dbGlb$tblName, "\n")
    ...
    isolate(if (input$mode == "New rows") {
      updateRadioButtons(
        session = session,
  ...
  observe({
    if (input$inputDBPan == "Map data") {
      ...
      if (!exists("prjs", envir = dbGlb, inherit = FALSE)) {
        ...
        delList <- c(
          "Unknown", "deprecated", "Unable to", "Unspecified", "Paris", "China",
      ...
      updateSelectInput(
        session = session, inputId = "mapUpLayers", choices = list(),
        selected = 0
      )
      epsg <- as.character(1:nrow(dbGlb$prjs))
      names(epsg) <- paste0("epsg:", dbGlb$prjs$code, " ", dbGlb$prjs$note)
      updateSelectInput(
        session = session, inputId = "mapUpSelectEPSG", choices = epsg,
        selected = 0
      )
  ...
  observe({
    if (is.null(input$mapUpload)) {
      return()
    }
    {
      cat(file = stderr(),"mapUpload\n")
      progress <- shiny::Progress$new(session, min = 1, max = 3)
      if (file.exists(input$mapUpload$datapath)) {
        fileEnding <- tolower(tools::file_ext(basename(input$mapUpload$datapath)))
        cat(file = stderr(),"mapUpload, filename=", input$mapUpload$datapath, " ending=", fileEnding, "\n")
        if (fileEnding != "zip") {
        ...
        cat(file = stderr(),"mapUpload, class(lyrs)=", class(lyrs), "\n")
        setwd(curdir)
        ...
        lyr <- unlist(lyr)
        updateSelectInput(
          session = session, inputId = "mapUpLayers", choices = lyrs,
          selected = lyr
        )
        progress$close()
      }
  ...
  observe({
    ...
    curdir <- getwd()
    setwd(datadir)
    cat(file = stderr(),"input$mapUpLayers =", input$mapUpLayers, "\n")
    datadir <- dir()
    ...
    cat(file = stderr(),"length(ids)=", length(ids), "\n")
    ...
    cat(file = stderr(),"input$mapUpLayers, number of layers (choices)=", length(choices), " selected=", selected, "\n")
    updateSelectInput(
      session = session, inputId = "mapUpIDMatch",
  ...
  observe({
    if (length(input$mapUpSelectEPSG)) {
      updateTextInput(
        session = session, inputId = "mapUpProjection",
  ...
  observe({
    if (input$mapUpSave > 0) {
      ...
      if (length(keep)) {
        ...
        output$mapActionMsg <- renderText(paste0(
          "Map saved for this project, StandID match=",
  ...
  customRunOps <- function() {
    isolate({
      if (length(input$runScript) == 0) {
        cat(file = stderr(),"in customRunOps runScript is empty\n")
        return()
      }
      cat(file = stderr(),"in customRunOps runScript: ", input$runScript, "\n")
  ...
  observe({
    if (input$toolsPan == "Manage projects") {
      ...
      updateSelectInput(
        session = session, inputId = "PrjSelect",
  ...
  observe(if (length(input$PrjSwitch) && input$PrjSwitch > 0) {
    cat(file = stderr(),"PrjSwitch to=", input$PrjSelect, "\n")
    isolate({
  ...
  saveRun <- function() {
    isolate({
      ...
      cat(file = stderr(),"leaving saveRun, saveTheRun=", saveTheRun, "\n")
    })
  }
})
